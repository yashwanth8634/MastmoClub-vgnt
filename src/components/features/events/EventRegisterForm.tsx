...
export default function EventRegisterForm({ event }: { event: EventType }) {
  ...
  useEffect(() => {
    if (event?.isTeamEvent) {
      ...
      setExtraMemberCount((prev) =>
        prev !== targetCount ? targetCount : prev,
      );
    }
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    for (let i = 0; i < extraMemberCount; i++) {
      ...
      const branch = formData.get(`member_branch_${i}`);
      const section = formData.get(`member_section_${i}`);
      if (name && roll && branch && section) {
        teamMembersArray.push({ name, rollNo: roll, branch, section });
      }
    }
    ...
    return (
      <div className="w-full max-w-2xl bg-black/80 backdrop-blur-xl border border-green-500/30 rounded-3xl p-10 text-center animate-in zoom-in shadow-[0_0_50px_rgba(0,255,0,0.15)]">
        ...
        <p className="text-gray-400 text-lg">
          ...
          <span className="text-[#00f0ff] font-bold">{event.title}</span>.
        </p>
        <button
          onClick={() => window.location.reload()}
          className="mt-8 px-8 py-3 bg-white/10 hover:bg-white/20 rounded-xl text-white font-bold transition-all hover:scale-105 border border-white/10 cursor-pointer"
        >
          Register Another
        </button>
      </div>
      ...
      <div className="text-center mb-10">
        ...
        {event.isTeamEvent ? (
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-purple-500/10 text-purple-300 text-xs font-bold uppercase tracking-wider border border-purple-500/20 shadow-[0_0_10px_rgba(168,85,247,0.2)]">
            <Users size={14} /> Team Event ({event.minTeamSize}-
          ...
          <div className="space-y-4 mt-2">
            ...
            <div className="grid grid-cols-3 gap-4">
              <div className="space-y-1">
                ...
                <select
                  name="year"
                  className="w-full bg-black border border-white/10 rounded-lg p-3 text-sm text-white focus:border-[#00f0ff] outline-none appearance-none cursor-pointer"
                >
                  {YEARS.map((y) => (
        ...
        {event.isTeamEvent && (
          <div className="space-y-4">
            {Array.from({ length: extraMemberCount }).map((_, index) => (
              <div
                key={index}
                className="p-5 bg-white/[0.03] border border-white/10 rounded-2xl relative animate-in fade-in slide-in-from-bottom-4 duration-500"
              >
        ...
        {event.isTeamEvent && (
          <div className="flex justify-center gap-4 pt-2">
            {extraMemberCount + 1 < event.maxTeamSize && (
              <button
                type="button"
                onClick={() => setExtraMemberCount((c) => c + 1)}
                className="flex items-center gap-2 text-[#00f0ff] bg-[#00f0ff]/5 border border-[#00f0ff]/20 px-5 py-2.5 rounded-full hover:bg-[#00f0ff]/10 text-xs font-bold uppercase tracking-wider transition-all cursor-pointer"
              >
            ...
            {extraMemberCount + 1 > (event.minTeamSize || 1) && (
              <button
                type="button"
                onClick={() => setExtraMemberCount((c) => c - 1)}
                className="flex items-center gap-2 text-red-400 bg-red-500/5 border border-red-500/20 px-5 py-2.5 rounded-full hover:bg-red-500/10 text-xs font-bold uppercase tracking-wider transition-all cursor-pointer"
              >
